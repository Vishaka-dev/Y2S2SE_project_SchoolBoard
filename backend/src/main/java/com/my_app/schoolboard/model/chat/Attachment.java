...
public class Attachment {
    ...
    private static final long MAX_FILE_SIZE = 52428800; // 50MB in bytes
    ...
    /**
     * Validate file size constraint (max 50MB)
     */
    @PrePersist
    @PreUpdate
    public void validateFileSize() {
        if (fileSize <= 0 || fileSize > MAX_FILE_SIZE) {
            throw new IllegalArgumentException(
                "File size must be between 1 byte and 50MB. Current: " + fileSize
            );
        }
    }
